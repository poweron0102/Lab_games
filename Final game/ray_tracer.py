...
class Ray_tracer:
    ...
    def ray_size(self):
        ...
        #rayX: cython.float
        #rayY: cython.float
        #playerX: cython.float
        #playerY: cython.float
        #player_ang: cython.float
        #aTan: cython.float
        #offsetX: cython.float
        #offsetY: cython.float
        #distH: cython.float
        #distV: cython.float
        #angle_ray: cython.float
        ...
        for contador in range(0, int(FOV * Rays_per_angle)):
            ...
            # print(f'{distH} \n {distV} \n \n -=-=-=-=-= \n \n')
            #pg.draw.circle(self.game.screen, 'green', ray_posiV, 2)
            #pg.draw.circle(self.game.screen, 'green', ray_posiH, 2)
    ...
    def draw(self, screen):
        ...
        for ray_atual, ray in enumerate(self.rays):
            line_higth = (Tile_size * Screen_distance / (ray[1] + 0.000001)) * self.game.map.wall_higth(ray[0][0], ray[0][1])
            ...
            pg.draw.rect(screen,  # Tela
                         ...
                         (ray_atual * const_screan, ((RES[1] / 2) - line_higth) / 2, const_screan, line_higth))# posição
